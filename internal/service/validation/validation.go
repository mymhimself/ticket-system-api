...
func (s *validationImpl) ChangeTicketStatus(requestBody *request.ChangeTicketStatus) error {
	err := validation.ValidateStruct(requestBody,
		validation.Field(&requestBody.Status, validation.Required),
		validation.Field(&requestBody.TicketID, validation.Required),
	)
	return err
}
